...
void draw() {
  if (dead==false) {
    background(255);
    ///// for debugging:
    //text("Falltiefe="+falltiefe,width/2,50);
...
void kollisionsKontrolle(){
  if (x[0]<=0|| x[0]>=SpielfeldGroesseX || y[0]<=-int(falltiefe/schrittweite) || y[0]>=SpielfeldGroesseY- int(falltiefe/schrittweite)) {
    dead = true;
  }
...
void neueFoodKoordinaten(int n) {
  if (n==tailLenght) {
    ...
    foodY=int(random(1,SpielfeldGroesseY-1))-int(falltiefe/schrittweite);
  }
...
void letGravityPull() {
  //falltiefe+=3;
  falltiefe+=int((schrittweite/2)- (schrittweite/2)/sqrt(0.2*score+1));
  translate(0,falltiefe);
  if (foodY>=SpielfeldGroesseY- int(falltiefe/schrittweite)) neueFoodKoordinaten(tailLenght);
}
